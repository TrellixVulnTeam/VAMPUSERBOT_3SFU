...
def button(page, modules):
    ...
    for pairs in pairs[page]:
        buttons.append(
            [
                custom.Button.inline(f"{VAMP_emoji} " + pair, data=f"Information[{page}]({pair})")
                for pair in pairs
            ]
        )
    ...
    buttons.append(
        [
            custom.Button.inline(
               f"⬅️ 𝐁𝐀𝐂𝐊 {VAMP_emoji}", data=f"page({(max_pages - 1) if page == 0 else (page - 1)})"
            ),
            custom.Button.inline(
               f"•{VAMP_emoji} ❌ {VAMP_emoji}•", data="close"
            ),
            custom.Button.inline(
               f"{VAMP_emoji} 𝐍𝐄𝐗𝐓 ➡️", data=f"page({0 if page == (max_pages - 1) else page + 1})"
            ),
        ]
    )
    return [max_pages, buttons]
    # Changing this line may give error in bot as i added some special cmds in VAMPBOT channel to get this module work...
...
if Var.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if event.query.user_id == bot.uid and query == "@VAMPBOT_OFFICIAL":
            ...
            result = builder.article(
                "@VAMPBOT_OFFICIAL",
                text="""**Hey! This is [VAMPBOT.](https://t.me/VAMPBOT_OFFICIAL) \nYou can know more about me from the links given below 👇**""",
                buttons=[
                    [
    ...
    async def page(event):
        if not event.query.user_id == bot.uid:
            return await event.answer(
                "Mil Gyi Tasalli..? Kabse mere bot me ungli kr rhe h. Khudka bna lo na agr chaiye to pta nhi kaha se aajate h disturb krne. ©VAMPBOT™ ",
                cache_time=0,
                alert=True,
            )
        ...
        await event.edit(
            f"**Legenday AF** [VAMPBOT](https://t.me/VAMPBOT_OFFICIAL) __Working...__\n\n**Number of modules installed :** `{len(CMD_HELP)}`\n**page:** {page + 1}/{veriler[0]}",
            buttons=veriler[1],
            link_preview=False,
        )
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:
            await delete_vamp(event,
              "⚜️VAMPBOT Menu Provider Is now Closed⚜️\n\n         **[©VAMPBOT](t.me/VAMPBOT_OFFICIAL)**", 5, link_preview=False
            )
        else:
            VAMP_alert = "Mil Gyi Tasalli..? Kabse mere bot me ungli kr rhe h. Khudka bna lo na agr chaiye to pta nhi kaha se aajate h disturb krne. ©VAMPBOT"
