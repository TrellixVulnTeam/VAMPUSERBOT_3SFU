...
async def set_not_afk(event):
    ...
    if "#" not in current_message and "yes" in USER_AFK:  # pylint:disable=E0602
        vampbot = await event.client.send_message(
            event.chat_id,
        ...
        try:
            await unsave_gif(event, vampbot)
        except:
            pass
        try:
...
async def _(event):
    ...
    owo = event.text[5:]
    reason = owo
    vamppic = await event.client.download_media(krakenop)
    if not USER_AFK:  # pylint:disable=E0602
        ...
        if owo == "":
            USER_AFK = f"yes: not-mentiond {vamppic}"  # pylint:disable=E0602
            x = await bot.send_message(
                event.chat_id, f"**I'm going afk🚶**", file=vamppic)
            try:
            ...
            try:
                xy = await bot.send_message(
                    Config.LOGGER_ID,
                    f"#AFKTRUE \n**AFK mode** = `True`\n**Reason:** `Not Mentioned`",file=vamppic
                    )
                try:
