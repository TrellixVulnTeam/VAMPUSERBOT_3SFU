...
David99q = str(ALIVE_NAME) if ALIVE_NAME else "vamp User"
papa = borg.uid
...
async def gban(userbot):
    ...
    if not sender.id == hum.id:
        ...
        VAMPBOT = await edit_or_reply(ids, "`Ok! Gban ho rha h intzarrr karo....`")
    hum = await userbot.client.get_me()
    await VAMPBOT.edit(f"`🔥Global Ban ho rha h ruko'💦....dekhte jaao bus kya hota h 😎 `")
    ...
    if user:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await VAMPBOT.edit(f"Gbaning ho rha h 😂\n\nTotal Chats :- `{a}`")
            except:
                b += 1
    else:
    ...
    try:
        if gmute(user.id) is False:
            return await VAMPBOT.edit(f"**Error! User phle se chuda(Gbanned) pda h 😂 .**")
    except:
        pass
    return await VAMPBOT.edit(
    ...
    if user:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await VAMPBOT.edit(f"Ok! Now Ungbaning ho rha h nhi to yhi patak kr chod dete.\nChats:- `{a}`")
            except:
                b += 1
    else:
